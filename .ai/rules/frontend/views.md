...
> Component-specific rules live in `components.md` (anonymous) and `php_components.md` (class-based).
...
- **MUST NOT** run queries or lazy-load relations from a view (no `Model::where(...)`, no `$post->comments` triggering a query inside `@foreach`). Receive already eager-loaded data from the controller — the N+1 problem. `Model::preventLazyLoading()` (see `../database/models.md`) turns any violation into an exception in dev.
...
- **MUST NOT** format dates with `Carbon::createFromFormat(...)` in the view — cast the column to `datetime` on the model (see `../database/models.md`) and format the Carbon instance: `{{ $order->ordered_at->format('m-d') }}`.
