...
> Refines `testing.md` for this directory. Behavioural tests never live here — see `feature.md` and `unit.md`.
...
```php
// app/Support — stateless, framework-agnostic helpers (../domain/support.md)
arch('support stays out of the HTTP layer')->expect('App\Support')->not->toUse('App\Http');

// app/Data — DTOs are immutable and hold no Eloquent (../domain/data.md)
...
// ../core/config.md — env() only inside config/*.php
