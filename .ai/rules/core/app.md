...
- **Controllers** already namespace by audience/version (`Api/`, `Admin/`, `Api/V{N}/`); domain nests *inside* that axis (`Api/V1/Billing/InvoiceController.php`) — see `../http/controllers.md`.
- **Factories** resolve from the model namespace, so a domain-namespaced model looks up `Database\Factories\Billing\InvoiceFactory` — mirror the subfolder (see `../database/factories.md`).
- **Migrations are the exception — keep them flat** (timestamp-ordered anonymous classes, not PSR-4; `migrate` ignores subfolders) — see `../database/migrations.md`.
...
- **MUST NOT** build HTML strings inside PHP classes (controllers, actions, services, mailables, notifications). Render markup in Blade and pass data in. (A Blade `render()` heredoc in a class-based component is the only sanctioned exception — see `../frontend/php_components.md`.)
...
- **MUST** depend on the interface, not the concrete, when one is bound — see `providers.md`.
...
- **MUST NOT** hard-code magic string/number literals for statuses, types, roles, or keys. Use an enum (`OrderStatus::Pending` — see `../domain/enums.md`) or a class constant (`Article::TYPE_NORMAL`).
...
- **MUST** read secrets only via `config(...)` (never `env()` outside `config/*.php`) — see `config.md`.
- **MUST NOT** mass-assign unvalidated request data; use Form Requests and `$fillable` / `$guarded` deliberately — see `../http/requests.md` and `../database/models.md`.

## Caching

Cache keys, `Cache::flexible` / locks / memo, tags, null results, and invalidation live in **`../domain/support.md`** — apply those rules at every call site (Jobs, Models, Actions), not only under Support classes.
