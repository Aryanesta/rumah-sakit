...
- The class that fires data onto a channel is an **Event** implementing `ShouldBroadcast` (queued) or `ShouldBroadcastNow` (synchronous) — see `events.md`. The `Broadcasting` directory only holds the auth gate.
...
> Register channel name patterns in `routes/channels.php` (see Registration above). Route file roles are listed in `../http/routes.md`.
