...
For complex/named limiters, define them in `App\Providers\AppServiceProvider::boot()` with `RateLimiter::for(...)` and reference by name (`throttle:uploads`). There is no `RouteServiceProvider` — see `middleware.md` for the `RateLimiter::for()` example.
...
> Middleware rules live in `middleware.md`.
