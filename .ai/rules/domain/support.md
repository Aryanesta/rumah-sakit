...
- **SHOULD** invalidate cache in model observers on `saved` / `deleted`. The observer is the right place because invalidation is cheap — but **MUST NOT** *recompute* the cache inside the observer (heavy work blocks the write path; see `../database/models.md`).
