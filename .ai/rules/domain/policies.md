...
- **Auth boundary:** session web routes use `auth` + policies/`->can(...)`. Token API routes use Sanctum (`auth:sanctum`) and **MUST** assert abilities on ability-scoped endpoints (grant + deny) — see `../testing/testing.md` and `../testing/feature.md`. Never treat a bare authenticated user as authorized for every ability.
