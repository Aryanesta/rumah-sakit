...
- **MUST** build the DTO at the boundary: a Form Request's `toDto()` (see `../http/requests.md`) or a named static constructor (`fromArray()`, `fromModel()`). Actions and Jobs receive a finished DTO; they don't assemble one from request input.
...
> Value objects produced by custom casts (`../database/casts.md`) also live here or in `app/Support/` — a DTO crosses a boundary, a value object models a single typed value.
