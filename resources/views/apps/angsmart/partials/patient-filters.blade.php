<div class="flex flex-col gap-3 lg:flex-row lg:items-center">
    ...
    <select x-model="phase" class="rounded-xl border-rs-border bg-rs-surface text-sm focus:border-rs-primary focus:ring-rs-primary">
        @foreach ($phaseOptions as $option)
            <option value="{{ $option }}">{{ $option }}</option>
        @endforeach
    </select>

    <select x-model="careStatus" class="rounded-xl border-rs-border bg-rs-surface text-sm focus:border-rs-primary focus:ring-rs-primary">
    ...
    <button
        type="button"
        @click="resetFilters()"
        class="rounded-xl border border-rs-border bg-rs-surface px-4 py-2.5 text-sm font-semibold text-rs-text-primary hover:bg-rs-background transition-colors"
    >
        Reset
    </button>
</div>
